...
| **4. Product-level field groups** (existing, actively-edited `product` CPT) | Dimensions/Care/Additional Information ACF group + `parts` Relationship field, all on the **same existing meta keys** JetEngine used; decide + implement the variation image gallery storage approach (options-table as-is vs. postmeta — see file `09` §3) | Every published product's existing Dimensions/Care/Additional Info/Parts values | **Highest caution of the four** — not because the data is "more real" than the others, but because `product` is edited daily by the team running the live store. Test against a **staging DB copy**, spot-check **several** real products (not just one), and confirm saving the edit screen doesn't rewrite or strip any sibling meta key it isn't supposed to touch |
...
- `context/existing-functionality/09-rebuild-considerations.md` §3 — the
  variation image gallery storage decision needed for chunk 4
