...
| `12A-woodmart-ajax-shop-filtering.md` | **How live shop filtering *feels* instant** — Woodmart `ajax_shop` + jQuery PJAX partial reload of `.wd-page-content` (filters + grid + counts in one server render). Two AJAX channels: PJAX for filter/sort/page-1, `woo_ajax` JSON for Load More append. Mandatory before choosing rebuild transport for Phase 3f. |
